...
### Verified
- `mkdocs build --strict` exits 0. The build emits the upstream Material for MkDocs informational banner about the future MkDocs 2.0 release — this is upstream output from the theme, not a project warning. The build also prints INFO-level notices for the 41 recipes that are intentionally accessed via the categorized [Recipes index](/agent-builder-field-guide/recipes/) rather than the top-level nav (see [ADR 0004](docs/decision-records/0004-recipe-nav-pattern.md)).
